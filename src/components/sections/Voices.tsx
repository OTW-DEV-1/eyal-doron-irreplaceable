...
const TESTIMONIALS: { name: string; role: string; img?: string; text: ReactNode }[] = [
  {
    name: 'עינת הר לוי',
    ...
    text: (
      <>
        ממליצה בחום על הקורס.
        <br />
        קורס מעולה.
        <br />
        יצאתי ממנו עם הרבה תובנות, כלים מעשיים ודרך חשיבה חדשה. במהלך הקורס עברתי תהליך משמעותי שבו רכשתי כלים
        והרגלים שאני בטוחה שילוו אותי בכל תחומי החיים. זה לא עוד קורס שנותן רק ידע אלא הוא משנה את הדרך שבה אנחנו
        מסתכלים על עצמנו ועל מה שאפשרי עבורנו. התכנים מונגשים באמצעות סדרת סרטונים מצוינת של ד״ר אייל דורון שמעביר את
        המסרים בצורה רהוטה, ברורה, מעוררת מחשבה ומאתגר אותנו להסתכל אחרת על היכולות והפוטנציאל שלנו. לפעמים כל מה
        שצריך הוא אדם אחד והחלטה אחת כדי להתחיל לחיות את החיים שבאמת רוצים. עבורי, הקורס הזה היה בדיוק זה.
      </>
    ),
  },
  {
    ...
    text: (
      <>
        הגעתי לקורס מתוך רצון לקבל ערך מוסף כמנהלת, וסיימתי עם הרבה יותר מזה.
        <br />
        מעבר לכלים המעשיים להגדרת מטרות, קבלת החלטות והובלת תהליכים הקורס העניק לי הזדמנות אמיתית לעצור, להתבונן
        בעצמי ולהתפתח כאשת מקצוע וכמנהלת. השילוב בין תכנים מקצועיים לבין תהליך של התפתחות אישית היה בעיניי הייחוד
        האמיתי של הקורס. אני ממליצה עליו בחום לכל מנהל בדרגי ביניים ומעלה שרוצה לא רק לנהל טוב יותר, אלא גם לצמוח
        ולהוביל מתוך מודעות, ביטחון ותחושת שליחות.
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    text: (
      <>
        השתחררתי לאחר 7 שנים בצבא. ביצעתי כמה תפקידי פיקוד בתור קצין עד שהחלטתי שאני רוצה למצוא את עצמי מחדש ולחפש
        כיוונים חדשים. כשהשתחררתי מהצבא חיפשתי מה בדיוק מעניין ומושך אותי.
        <br />
        הקורס נתן לי יכול להבין יותר מי אני ומה הן היכולות שלי. בעזרת כלים פרקטיים, פשוטים וקלים לביצוע, התקדמתי כמה
        צעדים בהבנה של מי אני ומה יש לי לתת.
        <br />
        אהבתי מאוד בקורס שהוא נותן באמת כלים פרקטים כמו טכניקות ושיטות כדי להגיע לפתרונות ולהבנות עמוקות.
      </>
    ),
  },
]
  ...
  return (
    <section id="voices" className="px-4 py-10 sm:px-[clamp(18px,6vw,8em)]">
      ...
      <div className="relative">
        <div
          ref={stripRef}
          className="no-scrollbar flex snap-x snap-proximity items-start gap-[22px] overflow-x-auto p-[6px_2px_14px] [scrollbar-width:none]"
        >
